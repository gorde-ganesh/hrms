...
describe('calculateLeaveDays', () => {
  it('counts a single weekday as 1', async () => {
    ...
    expect(await calculateLeaveDays(mon, mon)).toBe(1);
  });

  it('counts Mon–Fri as 5 days', async () => {
    expect(await calculateLeaveDays(new Date('2025-01-06'), new Date('2025-01-10'))).toBe(5);
  });

  it('excludes Saturday and Sunday from a full week span', async () => {
    // Mon 6 Jan to Sun 12 Jan = 5 working days
    expect(await calculateLeaveDays(new Date('2025-01-06'), new Date('2025-01-12'))).toBe(5);
  });

  it('returns 0 when start falls on a weekend and end is same day', async () => {
    const sat = new Date('2025-01-11'); // Saturday
    expect(await calculateLeaveDays(sat, sat)).toBe(0);
  });

  it('counts two separate weeks correctly', async () => {
    // Mon 6 Jan to Fri 17 Jan = 10 working days
    expect(await calculateLeaveDays(new Date('2025-01-06'), new Date('2025-01-17'))).toBe(10);
  });

  it('returns 1 for a Friday', async () => {
    const fri = new Date('2025-01-10');
    expect(await calculateLeaveDays(fri, fri)).toBe(1);
  });

  it('subtracts public holidays from working day count', async () => {
    const mp = (await import('../lib/prisma')).prisma as any;
    mp.holidayCalendar.findMany.mockResolvedValueOnce([
      { date: new Date('2025-01-06') }, // holiday on Monday
    ]);
    // Mon–Fri with Mon as holiday → 4 working days
    expect(await calculateLeaveDays(new Date('2025-01-06'), new Date('2025-01-10'))).toBe(4);
  });
});
